 ...
 * File /multicore_engine_core/include/mce/entity/parser/entity_template_lang_parser.hpp
 * Copyright 2015-2017 by Stefan Bodenschatz
 */

#ifndef ENTITY_PARSER_ENTITY_TEMPLATE_LANG_PARSER_HPP_
#define ENTITY_PARSER_ENTITY_TEMPLATE_LANG_PARSER_HPP_

/**
 * \file
 * Defines the parser class for the entity template language.
 */
...
/// Implements the back-end of the entity template language parser.
/**
...
class entity_template_lang_parser_backend {
	...
	struct ast_definition_visitor : boost::static_visitor<> {
		entity_template_lang_parser_backend& backend;
		...
		explicit ast_definition_visitor(entity_template_lang_parser_backend& backend) : backend(backend) {}
	};
	struct ast_instance_visitor : boost::static_visitor<> {
		...
		void operator()(ast::include_instruction& node);
		void operator()(const ast::entity_instance& node);
		explicit ast_instance_visitor(entity_template_lang_parser_backend& backend) : backend(backend) {}
	};
	struct ast_position_visitor : boost::static_visitor<entity_position_t> {
		...
		explicit ast_position_visitor(entity_template_lang_parser_backend& backend) : backend(backend) {}
	};
	struct ast_orientation_visitor : boost::static_visitor<entity_orientation_t> {
		...
		explicit ast_orientation_visitor(entity_template_lang_parser_backend& backend) : backend(backend) {}
	};
	...
	ast::ast_root load_file(const asset::asset_ptr& template_lang_file_asset);

public:
	/// Constructs a parser back-end for the given entity_manager.
	explicit entity_template_lang_parser_backend(entity_manager& em) : em(em) {}
	/// Loads and processes the file represented by the given asset.
	void load_and_process_file(const asset::asset_ptr& template_lang_file_asset);
};
...
#endif /* ENTITY_PARSER_ENTITY_TEMPLATE_LANG_PARSER_HPP_ */
