...
template <typename Root_Type, template <typename> class Abstract_Assignment>
class abstract_property<Root_Type, Abstract_Assignment, void> {
protected:
	std::string name_;
...
public:
	...
	virtual ~abstract_property() = default;
	virtual mce::reflection::type_t type() const noexcept = 0;
	virtual std::unique_ptr<Abstract_Assignment<Root_Type>> make_assignment() const = 0;
	const std::string& name() const {
		return name_;
	}
	// TODO: Implement interface for binary serialization of objects
...
template <typename Root_Type, typename T,
		  ...
		  template <typename, typename> class Assignment = null_assignment, typename Assignment_Param = void>
...
class property<Root_Type, T, Abstract_Assignment, Assignment, void>
		: public abstract_property<Root_Type, Abstract_Assignment, void> {
public:
	typedef typename detail::property_type_helper<T, void>::accessor_value accessor_value;
	property(const std::string& name) : abstract_property<Root_Type, Abstract_Assignment>(name) {}
	property(const property&) = delete;
	property(property&&) = delete;
	property& operator=(const property&) = delete;
	property& operator=(property&&) = delete;
	...
	virtual accessor_value get_value(const Root_Type& object) const = 0;
	virtual void set_value(Root_Type& object, accessor_value value) const = 0;
	virtual std::unique_ptr<Abstract_Assignment<Root_Type>> make_assignment() const override;
};
...
template <typename Root_Type, typename T, template <typename> class AbstractAssignment,
		  template <typename, typename> class Assignment>
std::unique_ptr<AbstractAssignment<Root_Type>>
property<Root_Type, T, AbstractAssignment, Assignment, void>::make_assignment() const {
...
public:
	linked_property(const std::string& name, getter_t getter, setter_t setter)
			: property<Root_Type, T, AbstractAssignment, Assignment, Assignment_Param>(name), getter(getter),
			  setter(setter) {}
...
template <typename Root_Type, template <typename> class AbstractAssignment,
		  template <typename, typename> class Assignment, typename Assignment_Param = void, typename T,
