...
#include <mce/asset_gen/load_unit_description_ast_fusion.hpp>
...
namespace asset_gen {
namespace parser {
namespace load_unit_description_parser_impl {
using x3::char_;
using x3::eoi;
using x3::eol;
using x3::lexeme;
...
x3::rule<class load_unit_ast_root, ast::load_unit_ast_root> start = "start";
x3::rule<class load_unit_section, ast::load_unit_section> section = "section";
x3::rule<class load_unit_entry, ast::load_unit_entry> entry = "entry";
x3::rule<class load_unit_string_literal, std::string> string_literal = "string_literal";
x3::rule<class load_unit_identifier, std::string> identifier = "identifier";
x3::rule<class load_unit_lookup_spec, ast::lookup_type> lookup_spec = "lookup_spec";
...
auto section_def = identifier > lit('{') > *(entry) > lit('}');
...
ast::load_unit_ast_root load_unit_description_parser::parse(const std::string& filename, const char*& first,
	...
	try {
		bool r = phrase_parse(first, last, load_unit_description_parser_impl::start,
							  load_unit_description_parser_impl::skipper, ast_root);
