...
TEST(model_collision_model_test, load_example_collision_model) {
	...
	auto loader = std::make_shared<asset::load_unit_asset_loader>(std::vector<asset::path_prefix>(
			{{std::make_unique<asset::pack_file_reader>(), "../multicore_engine_test_assets/assets.pack"},
			 {std::make_unique<asset::pack_file_reader>(),
			  "../../multicore_engine_test_assets/assets.pack"}}));
	am.add_asset_loader(loader);
	std::atomic<bool> load_unit_ready{false};
	std::atomic<bool> load_unit_failed{false};
	std::exception_ptr ex_ptr;
	am.start_pin_load_unit(
			"models_col", [&load_unit_ready]() { load_unit_ready = true; },
			[&load_unit_failed, &ex_ptr](std::exception_ptr ex) {
				ex_ptr = ex;
				load_unit_failed = true;
			});
	...
	mm.load_collision_model(
			"models/cube", [&model_loaded](const collision_model_ptr&) { model_loaded = true; },
...
TEST(model_collision_model_test, load_example_polygon_model) {
	...
	std::exception_ptr ex_ptr;
	am.start_pin_load_unit(
			"models_geo", [&load_unit_ready]() { load_unit_ready = true; },
			[&load_unit_failed, &ex_ptr](std::exception_ptr ex) {
				ex_ptr = ex;
				load_unit_failed = true;
			});
	...
	std::atomic<bool> model_loaded{false};
	std::atomic<bool> model_failed{false};
	mm.load_polygon_model(
			"models/cube", [&model_loaded](const polygon_model_ptr&) { model_loaded = true; },
			[&model_failed, &ex_ptr](std::exception_ptr ex) {
				ex_ptr = ex;
				model_failed = true;
			});
	while(!model_loaded && !model_failed) {
