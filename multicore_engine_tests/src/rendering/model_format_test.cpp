...
TEST(rendering_model_format_test, expected_vertex_input_config) {
	...
	ASSERT_EQ(0u, conf.bindings().at(0).binding);
	ASSERT_EQ(5u, conf.attributes().size());
	ASSERT_EQ(0u, conf.attributes().at(0).binding);
	ASSERT_EQ(0u, conf.attributes().at(1).binding);
	ASSERT_EQ(0u, conf.attributes().at(2).binding);
	ASSERT_EQ(0u, conf.attributes().at(3).binding);
	ASSERT_EQ(0u, conf.attributes().at(4).binding);
	ASSERT_EQ(0u, conf.attributes().at(0).location);
	ASSERT_EQ(1u, conf.attributes().at(1).location);
	ASSERT_EQ(2u, conf.attributes().at(2).location);
	ASSERT_EQ(3u, conf.attributes().at(3).location);
	ASSERT_EQ(4u, conf.attributes().at(4).location);
