 ...
 * File /multicore_engine_headed/src/input/first_person_input_state.cpp
...
first_person_input_state::first_person_input_state(input_system* system, core::game_state*)
		: system_state(system) {
	system->win_sys.window().cursor_mode(glfw::cursor_mode::disabled);
}

void first_person_input_state::reenter(const boost::any&) {
	static_cast<input_system*>(system_)->win_sys.window().cursor_mode(glfw::cursor_mode::disabled);
}

first_person_input_state::~first_person_input_state() {}

void first_person_input_state::register_to_entity_manager(entity::entity_manager& em) {
	REGISTER_COMPONENT_TYPE_SIMPLE(em, first_person_flyer,
								   this->create_first_person_flyer_component(owner, config), this);
}

void first_person_input_state::process(const mce::core::frame_time& frame_time) {
