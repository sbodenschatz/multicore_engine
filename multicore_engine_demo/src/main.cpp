...
void test_run(int runs, int objects, mce::containers::unordered_object_pool<X, block_size>& uop,
			  int outer_run) {
	for(int run = 0; run < runs; ++run) {
		if(run % ((runs / 10) ? runs / 10 : 1) == 0) std::cout << outer_run << " " << run;
		for(int i = 0; i < objects; ++i) uop.emplace(i);
		...
		for(auto& v : uop) { std::cout << v.v << std::endl; }
	}
}
